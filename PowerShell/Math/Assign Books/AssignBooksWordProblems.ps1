...
#----------------------------------------------------------------------------------------------------------
$env:UserName
$target = "C:\Users\$env:UserName\Mr. Ansh\PowerShell and Batch\Ansh Jain\Word Problems"
...
#Objective 4: Manipulate one of the file names to obtain a variable that is just "Word Problems 1A"
...
foreach ($file in $sourceFolder) {
    if(($file -like '*TEST*') -or ($file -like '*BOOK*')) {
        ...
        $endIntString = $endIntString.Trim(" ")
...
foreach($file in $sourceFolder) {
    ...
    $index = $startIntString.IndexOf(".") - 1
    $endIntStringArray = $startIntString[0..$index]
    $endIntString = ""
    foreach ($portion in $endIntStringArray) {
        $endIntString = $endIntString + $portion
    }
    $endIntString = $endIntString.Trim(" ")
        if($endIntString -eq [string]$nextPage) {
            #Write-Host $file
            $sourceFile = $file
            break
        }
